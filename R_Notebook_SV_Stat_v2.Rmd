...
t.test(group_one, group_two, var.equal = TRUE)
```
...
Lastly, we use one-sample test to determine whether the median of one sample deviates from a known value (in this case, 15):
```{r}
sample <- c(1,2,3,4,5,10,20,30)
wilcox.test(sample, mu = 15)
```
...
anova_result <- aov(value ~ group1 * group2, data = data)
# The only difference between the two types is whether you set your value up with only one group, or two. 
```

## Linear Regressions
Compared with the last three tests, linear regressions are simple to perform in R. Keep in mind that there are multiple steps, however; We can write an instance of this out as:
```{r}
# Load in your data
...
model_mult <- lm(y ~ x1 + x2 + x3, data = data)
```
...
Finally, we can use this test to predict new values with the help of the predict() function. We can achieve this through writing:
```{r}
new_data <- data.frame(x = c(10, 20, 30))  # Replace whatever is in c() with your own values
...
We can set up logical systems to facilitate the integration of these elements in R. We can try out a way to do this with both of them in one code block, since they can only exist in the context of each other. Here's an example of what we can do:
```{r}
# Null Hypothesis: There is no difference.
# Alternative Hypothesis: There is a difference.

# Obtaining a p-value from example data:
# pnorm is a function indicating the use of normal distribution.
z_score <- (sample_mean - population_mean) / standard_error
p_value <- 2 * pnorm(-abs(z_score))  # This is for a two-tailed test

# Setting up a basic logic system:
if(p_value <= 0.05) {
  result <- print("The results indicate rejection of the Null Hypothesis!")
} else if(p_value > 0.05) {
  result <- print("The results indicate acceptance of the Null Hypothesis!")
}
result
```
