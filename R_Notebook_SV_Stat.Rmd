...
-   V4: Combined general information and code blocks into the same section.
    -   V4.1: Added some more text for the preface and end material, as well as a new "Useful Packages in R" section in the preface.
...
In addition to a general review of these elements, I will be covering blocks of R code that correspond to each element. This way, I can refer to this notebook in the likely case that I need to run a statistical test in R. These will come at the end of each test's and data visualizsation's section.

## Useful Packages in R

Base R is very helpful on its own for gathering and analyzing data, but sometimes we prefer to quicken the process by installing some external R code, known as **packages**. Two of the more prominent ones we like to use include:

-   **ggplot2**: Used primarily to enhance data visualization with extra customization options.
-   **plotrix**: Used for increasing R's range of statistical functions, saving extra time.

# The Primary Statistical Tests

As mentioned before, the primary statistical tests are the t-test, Wilcoxon test, ANOVA, linear regression and correlation tests. These are the tools with which a data analyst will derive meaningful information from raw data. Statisticians can use these tests to gather vital information in order to piece graphs and results together.
...
It was initially created by **William S. Gosset**, an English statistician employed at the Guinness Brewery in Dublin. By the 1970s, this test was available in most software statistical packages.[^1]

[^1]: <https://www.statisticshowto.com/probability-and-statistics/t-test/#assump>
...
R Studio stands out among other programming languages in its ability to perform data analysis tests. Here, we can insert a code block with a method of performing a t-test.
...
The **Wilcoxon test** (a.k.a. Wilcoxon signed rank test) is similar to the t-test, with one significant difference: it is **nonparametric**. This means that the test doesn't assume that the data follows a distribution.[^2]

[^2]: <https://statisticsbyjim.com/hypothesis-testing/wilcoxon-signed-rank-test/>

This test was created by **Frank Wilcoxon** in 1945, in the same scientific paper that also presented the rank-sum test. It was popularized by American psychologist and statistician Sidney Siegel in 1965.[^3]

[^3]: <https://online.stat.psu.edu/stat415/lesson/20/20.2>
...
The **ANOVA** (a.k.a. Analysis of Variance) test is used to see of the means of **three or more groups** for significant differences among them.[^4] This assists statisticians with determining whether differences observed in the groups' means are due to chance or caused by true effects of the property applied to them. This makes it similar to the t-test, only it has the capacity for 3+ groups' means.

[^4]: <https://researchmethod.net/anova/>

This test was devised by **R. A. Fisher**, a British statistician, in 1918. It was developed using the t-test as a reference point, which explains its similarities.[^5]

[^5]: <https://www.britannica.com/topic/variance-analysis-statistics>
...
**Linear regression** is a statistical model that details the relationships between at least one **explanatory variable** (any variable that predicts changes in another variable[^6]) and an **outcome variable** (the variable that's being affected by the explanatory variable[^7]). [^8]

[^6]: <https://statisticsbyjim.com/glossary/explanatory-variable/>

[^7]: <https://statisticsbyjim.com/glossary/response-variable/>

[^8]: <https://statisticsbyjim.com/regression/linear-regression/#google_vignette>
...
-   The null and alternative hypotheses of the linear regression analysis are:
    -   Ho: The slope of the population equals zero (in the formula y=β₀+β₁+ε ). The hypotheses focus on the status of β₁ in this instance[^9]
    -   Ha: The slope of the population doesn't equal zero

[^9]: [https://stats.libretexts.org/Bookshelves/Introductory_Statistics/Mostly_Harmless_Statistics\_(Webb)/12%3A_Correlation_and_Regression/12.02%3A_Simple_Linear_Regression/12.2.01%3A_Hypothesis_Test_for_Linear_Regression](https://stats.libretexts.org/Bookshelves/Introductory_Statistics/Mostly_Harmless_Statistics_(Webb)/12%3A_Correlation_and_Regression/12.02%3A_Simple_Linear_Regression/12.2.01%3A_Hypothesis_Test_for_Linear_Regression){.uri}

### Coding Process

For performing the linear regression, we will use a base data set, "mtcars" for the purpose of this exercise. Keep in mind that there are multiple steps, however; We can write an instance of this out as:
...
The **correlation test** (a.k.a. correlation hypothesis test) is used to determine the strength and direction of the relationship between two variables.[^10] This statistical test does this through use of the correlation coefficient and sample size.

[^10]: <https://stats.libretexts.org/Courses/Rio_Hondo_College/Math_130%3A_Statistics/11%3A_Correlation/11.03%3A_Correlation_Hypothesis_Test>

### Assumptions

-   When using this test, we assume:
    -   Normal data distribution
    -   A linear relationship between two variables of interest[^11]
    -   Minimal outliers

[^11]: <https://usq.pressbooks.pub/statisticsforresearchstudents/chapter/correlation-assumptions/>

### When to Use It

We use the correlation hypothesis test when we want to determine whether the relationship between two non-outcome variables is significant or not.[^12] This makes it similarly useful in clinical trials, compared to linear regression analyses.

[^12]: <https://www.statology.org/when-to-use-correlation/>
...
There are three methods we can use when performing correlation tests in R: 'pearson', 'spearman' and 'kendall'. We use the pearson method when we're dealing with **linear relationships** between target variables, the spearman method when we're dealing with **monotonic**[^13] relationships, and the kendall method for **ordinal data**[^14] . We can write these out as:

[^13]: **Both variables have the same direction, but rates of change may differ.**

[^14]: **Any data that has a natural, ordered sequence.**
...
Here is a link to an image of a typical histogram: <https://www.geeksforgeeks.org/maths/histogram/>

### Terms to Know

-   Statistical entities associated with histograms include:
    -   **Central tendency**: Which value the majority of recorded samples fall into. This is represented as a peak in the histogram. There can be more than one peak, depending on data.[^18]
...
[^18]: <https://statisticsbyjim.com/basics/histograms/>
...
Box plots are very similar to histograms, except that multiple intervals can be displayed in one graph. This makes it useful for researchers, as it can display a lot of information in a single image.[^19]. It's basically a simplified version of the data presented in multiple histograms. Box plots don't make assumptions on the distribution of the data, making it nonparametric.

[^19]: <https://statisticsbyjim.com/graphs/box-plot/>
...
Heat maps are typically used with large sets of data, all with values that can differ to various intensities.[^20] This data visualization technique will chart the data in a color-coded fashion, according to a spectrum of values. Heat maps are typically displayed in a **2d or 3d** matrix, and use a range of values as a reference point for a color spectrum.

[^20]: <https://www.geeksforgeeks.org/data-visualization/what-is-heatmap-data-visualization-and-how-to-use-it/>
...
Here is an example of a typical 2-dimensional heat map... [https://www.researchgate.net/publication/344202320/figure/fig5/AS:959980165414946\@1605888515416/A-heat-map-of-the-two-dimensional-hierarchical-cluster-analysis-suggesting-differential.png](https://www.researchgate.net/publication/344202320/figure/fig5/AS:959980165414946@1605888515416/A-heat-map-of-the-two-dimensional-hierarchical-cluster-analysis-suggesting-differential.png){.uri}
...
Pie charts are likely one of the first data visualization methods one will use. This can be chalked up to their ease of use; However, they tend to be one of the most misused. [^21] A pie chart consists of a circle, with categorical data being represented on it as "slices." Larger slices mean it takes up a larger portion of a single population/variable.

[^21]: <https://www.storytellingwithdata.com/blog/2020/5/14/what-is-a-pie-chart>
...
In this instance, the pie chart will have differently-colored slices, the number of colors corresponding to the number of slices. This was indicated by the "col = rainbow(length(slices))" argument within the function.
...
While base R has plenty of methods by which we can organize and evaluate raw data, it can be helpful to go ahead and install some packages, such as ggplot2 and plotrix. This way, we aren't spending as much time creating a formula for a specific statistical test from scratch.
